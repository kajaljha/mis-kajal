...
class ReportPdf < Prawn::Document
  ...
  def header
    ...
    image "#{Rails.root}/app/assets/images/MIS_Logo.png", width: 530, height: 150
  end
  ...
  def table_content
    ...
    table product_rows do
      ...
      self.column_widths = [50, 100, 100, 100, 100, 100]
    end
  end

  def product_rows
    [['#', 'Invoice Number', 'Bill To', 'Currency']] +
      @invoices.map do |invoice|
      [invoice.id, invoice.invoice_number, invoice.bill_to, invoice.currency]
    end
  end
# invoice_date
#      :invoice_number
